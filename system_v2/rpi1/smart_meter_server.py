...
class SmartMeterDataBlock(ModbusSequentialDataBlock):
    ...
    def getValues(self, address, count=1):
        ...
        if address == 0 and count == 8:
            self.total_served += 1
            logger.info(
                f"[TX TO OPTA] Served registers 0-7 | "
...
async def main():
    ...
    """
    logger.info("=" * 80)
    logger.info("RPI#1 - Smart Meter / RTU Server Starting")
    logger.info("=" * 80)
    logger.info(f"Architecture:")
    logger.info(f"  ESP32 → RPI#1: WiFi, Modbus TCP (writes data)")
    logger.info(f"  RPI#1 ← Opta: Ethernet, Modbus TCP (reads data)")
    logger.info(f"")
    logger.info(f"Configuration:")
    logger.info(f"  TCP Server: {BIND_ADDRESS}:{BIND_PORT}")
    logger.info(f"  Unit ID: {UNIT_ID}")
    logger.info(f"  Interfaces: WiFi  + Ethernet (192.168.2.100)")
    logger.info("=" * 80)
    ...
    logger.info("  - ESP32 to write data (WiFi interface)")
    logger.info("  - Opta to read data (Ethernet interface)")
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("Smart Meter Server Shutdown")
        logger.info("=" * 80)
